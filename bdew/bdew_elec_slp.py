...
class ElecSlp:
    ...
    def __init__(self, year, seasons=None, holidays=None):
        if calendar.isleap(year):
            #checks if the year is bisexto and if true then we have plus 24 hours
            #variable hoy is the number of hours in the year
            hoy = 8784
        else:
            hoy = 8760

        #gets the name of the directory of this python file and adds the file "bdew_data" to this directory as well
        self.datapath = os.path.join(os.path.dirname(__file__), "bdew_data")

        #gives back a list with equally spaced time points
        # in this case our time list starts at the first day of the first month of the chosen year
        #and goes till the last day of the year with a frequency from 15min
        ...
        self.final_slp_frame = self.create_dynamic_h0_profile()
    ...
    def create_bdew_load_profiles(self, dt_index, slp_types, holidays=None):
        ...
        #ele soma primeiro as colunas, ou seja soma o consumo anual de cada tipo de slp
        #depois ele divide a cada valor da tabela pela soma anual
        #nao entendi porque
    ...
    def get_profile(self, ann_el_demand_per_sector, slp_frame):
